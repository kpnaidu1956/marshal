...
use ruvector_core::{
    ...
    DistanceMetric, RuvectorError, SearchQuery, SearchResult, VectorDB as CoreVectorDB, VectorEntry,
};
use std::sync::Arc;
use std::sync::RwLock;
...
// Import new crates
use ruvector_collections::{CollectionError, CollectionManager as CoreCollectionManager};
...
impl VectorDB {
    ...
    pub async fn insert(&self, entry: JsVectorEntry) -> Result<String> {
        ...
        tokio::task::spawn_blocking(move || {
            let db = db.read().map_err(|_| RuvectorError::Internal("Internal state error".into()))?;
...
impl CollectionManager {
    ...
    pub async fn create_collection(&self, name: String, config: JsCollectionConfig) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || {
            let manager = manager.write().map_err(|_| CollectionError::InvalidConfiguration { message: "Internal state error".into() })?;
    ...
    pub async fn list_collections(&self) -> Result<Vec<String>> {
        ...
        tokio::task::spawn_blocking(move || {
            let manager = manager.read().map_err(|_| Error::from_reason("Internal state error"))?;
            Ok(manager.list_collections())
        })
        .await
        .map_err(|e| Error::from_reason(format!("Task failed: {}", e)))?
    }
    ...
    pub async fn get_stats(&self, name: String) -> Result<JsCollectionStats> {
        ...
        tokio::task::spawn_blocking(move || {
            let manager = manager.read().map_err(|_| CollectionError::InvalidConfiguration { message: "Internal state error".into() })?;
    ...
    pub async fn list_aliases(&self) -> Result<Vec<JsAlias>> {
        ...
        let aliases = tokio::task::spawn_blocking(move || {
            let manager = manager.read().map_err(|_| Error::from_reason("Internal state error"))?;
            Ok::<_, napi::Error>(manager.list_aliases())
        })
        .await
        .map_err(|e| Error::from_reason(format!("Task failed: {}", e)))??;
