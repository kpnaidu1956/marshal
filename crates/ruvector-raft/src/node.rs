...
pub struct RaftNode {
    ...
    internal_rx: Arc<RwLock<mpsc::Receiver<InternalMessage>>>,
}
...
impl RaftNode {
    ...
    async fn handle_client_command(
        ...
        drop(persistent);
        let _ = self.internal_tx.send(InternalMessage::HeartbeatTimeout).await;
    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if node.election_state.read().should_start_election() {
                    let _ = node.internal_tx.send(InternalMessage::ElectionTimeout).await;
                }
            }
        });
    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if node.state.read().is_leader() {
                    let _ = node.internal_tx.send(InternalMessage::HeartbeatTimeout).await;
                }
            }
        });
    }
