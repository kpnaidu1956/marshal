...
impl StreamManager {
    ...
    /// Remove a stream by consumer ID (prevents memory leak)
    pub fn remove_stream(&self, consumer_id: &str) {
        self.streams.write().retain(|s| s.consumer_id != consumer_id);
    }

    /// Remove all streams that have no external references (only the manager holds them)
    pub fn cleanup_unused(&self) {
        self.streams.write().retain(|s| Arc::strong_count(s) > 1);
    }
