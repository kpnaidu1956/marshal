...
impl LogEntry {
    ...
    /// Calculate checksum for data (FNV-1a — stable across Rust versions unlike DefaultHasher)
...
impl ReplicationLog {
    ...
    /// Maximum log entries before auto-truncation
    ...
    pub fn append(&self, data: Vec<u8>) -> LogEntry {
        ...
        *seq += 1;
        ...
        if self.entries.len() > Self::MAX_LOG_SIZE {
            let cutoff = *seq - (Self::MAX_LOG_SIZE as u64 / 2);
