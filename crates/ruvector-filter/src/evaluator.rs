...
impl<'a> FilterEvaluator<'a> {
    ...
    fn evaluate_is_null(&self, field: &str) -> Result<HashSet<String>> {
        // IsNull requires knowing the full document universe to compute the complement.
        // Since the filter index only tracks non-null values, we cannot determine which
        // documents are missing a field without a full document registry.
        // Return an error instead of silently returning empty (which was a bug).
        Err(FilterError::UnsupportedOperation(format!(
            "IsNull filter on '{}' is not supported — the index does not track null values", field
        )))
    }
